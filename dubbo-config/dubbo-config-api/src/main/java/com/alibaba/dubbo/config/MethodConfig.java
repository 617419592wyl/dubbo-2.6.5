...
public class MethodConfig extends AbstractMethodConfig {
    ...
    // whether to retry 是否重试
    private Boolean retry;

    // if it's reliable
    private Boolean reliable;

    // thread limits for method invocations 方法provider执行的并发数限制
    ...
    // whether need to return 是否需要返回
    private Boolean isReturn;

    // callback instance when async-call is invoked 异步执行回调对象
    private Object oninvoke;

    // callback method when async-call is invoked 异步执行回调方法
    private String oninvokeMethod;

    // callback instance when async-call is returned 异步回调返回对象
    private Object onreturn;

    // callback method when async-call is returned 异步回调返回方法
    private String onreturnMethod;

    // callback instance when async-call has exception thrown 回调对象什么异常抛出
    private Object onthrow;

    // callback method when async-call has exception thrown 回调方法什么异常抛出
