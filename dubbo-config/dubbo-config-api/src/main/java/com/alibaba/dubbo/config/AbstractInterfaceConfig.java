...
//            如果注册中心没有配置获取获取系统属性dubbo.registry.address属性值
            ...
            if (address != null && address.length() > 0) {
                ...
                String[] as = address.split("\\s*[|]+\\s*");//多个注册地址用|分开
                for (String a : as) {
//                    初始化注册中心配置
...
//            设置注册中心的系统属性值=》
    ...
    protected void checkApplication() {
        // for backward compatibility
        if (application == null) {
//            如果没有应用配置，从dubbo.application.name 系统属性值
...
//        从系统变量读取dubbo.service.shutdown.wait属性值，服务关闭等待时间
        String wait = ConfigUtils.getProperty(Constants.SHUTDOWN_WAIT_KEY);
        if (wait != null && wait.trim().length() > 0) {
            System.setProperty(Constants.SHUTDOWN_WAIT_KEY, wait.trim());
        } else {
//            服务关闭等到时间获取系统属性dubbo.service.shutdown.wait.seconds 服务关闭时等待多少s
...
//                        可以注册不订阅
    ...
    protected URL loadMonitor(URL registryURL) {
        if (monitor == null) {
//            查询系统变量dubbo.monitor.address属性值
            String monitorAddress = ConfigUtils.getProperty("dubbo.monitor.address");
//            查询系统变量dubbo.monitor.protocol属性值
...
//        timestamp
        ...
        //set ip 查询系统变量DUBBO_IP_TO_REGISTRY值
...
//        查询系统变量dubbo.monitor.address值
    ...
    protected void checkInterfaceAndMethods(Class<?> interfaceClass, List<MethodConfig> methods) {
        ...
        // to verify interfaceClass is an interface 验证接口是否是一个接口
        if (!interfaceClass.isInterface()) {
            throw new IllegalStateException("The interface class " + interfaceClass + " is not a interface!");
        }
        // check if methods exist in the interface 检查方法是否是接口的方法
