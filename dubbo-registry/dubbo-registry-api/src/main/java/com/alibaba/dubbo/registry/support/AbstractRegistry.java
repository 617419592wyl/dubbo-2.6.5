...
public abstract class AbstractRegistry implements Registry {
    ...
    // URL address separator, used in file cache, service provider URL separation URL地址分隔符，用于文件缓存，服务提供商的URL分隔
    private static final char URL_SEPARATOR = ' ';
    // URL address separated regular expression for parsing the service provider URL list in the file cache URL地址分隔正则表达式，用于解析文件缓存中的服务提供者URL列表
    private static final String URL_SPLIT = "\\s+";
    // Log output
    protected final Logger logger = LoggerFactory.getLogger(getClass());
    // Local disk cache, where the special key value.registies records the list of registry centers, and the others are the list of notified service providers 本地磁盘缓存，其中有特殊键值。registies记录注册中心的列表，其他的是通知服务提供者的列表
