...
class CallbackServiceCodec {
    ...
    private static String exportOrunexportCallbackService(Channel channel, URL url, Class clazz, Object inst, Boolean export) throws IOException {
        ...
        // add method, for verifying against method, automatic fallback (see dubbo protocol) 添加方法，用于验证方法，自动回退(参见dubbo协议)
        ...
        // no need to generate multiple exporters for different channel in the same JVM, cache key cannot collide. 无需为同一JVM中的不同通道生成多个导出器，缓存键不会碰撞。
        String cacheKey = getClientSideCallbackServiceCacheKey(instid);
        String countkey = getClientSideCountKey(clazz.getName());
        if (export) {
            // one channel can have multiple callback instances, no need to re-export for different instance. 一个通道可以有多个回调实例，不需要为不同的实例重新导出。
            if (!channel.hasAttribute(cacheKey)) {
                if (!isInstancesOverLimit(channel, url, clazz.getName(), instid, false)) {
                    Invoker<?> invoker = proxyFactory.getInvoker(inst, clazz, exporturl);
                    // should destroy resource?
                    Exporter<?> exporter = protocol.export(invoker);
                    // this is used for tracing if instid has published service or not. 这用于跟踪是否已经发布了服务。
    ...
    /**
     * refer or destroy callback service on server side 引用或销毁服务器端回调服务
     *
     * @param url
     */
    ...
    public static Object decodeInvocationArgument(Channel channel, RpcInvocation inv, Class<?>[] pts, int paraIndex, Object inObject) throws IOException {
        // if it's a callback, create proxy on client side, callback interface on client side can be invoked through channel 如果是回调，在客户端创建代理，客户端回调接口可以通过通道调用
