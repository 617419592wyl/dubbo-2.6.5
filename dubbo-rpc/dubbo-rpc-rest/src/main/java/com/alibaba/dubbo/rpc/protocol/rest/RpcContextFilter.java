...
public class RpcContextFilter implements ContainerRequestFilter, ClientRequestFilter {
    ...
    // currently we use a single header to hold the attachments so that the total attachment size limit is about 8k 目前我们使用单个标题来保存附件，因此附件的总大小限制大约为8k
    ...
    public void filter(ClientRequestContext requestContext) throws IOException {
        ...
        for (Map.Entry<String, String> entry : RpcContext.getContext().getAttachments().entrySet()) {
            ...
            // TODO for now we don't consider the differences of encoding and server limit 目前我们不考虑编码和服务器限制的差异
