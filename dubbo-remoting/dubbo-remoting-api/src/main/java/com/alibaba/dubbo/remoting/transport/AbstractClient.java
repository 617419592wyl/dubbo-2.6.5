...
public abstract class AbstractClient extends AbstractEndpoint implements Client {
    ...
    // Reconnection error log has been called before? 重连接错误日志以前调用过吗?
    private final AtomicBoolean reconnect_error_log_flag = new AtomicBoolean(false);
    // reconnect warning period. Reconnect warning interval (log warning after how many times) //for test 连接的警告。重新连接警告间隔(日志警告多少次后)//进行测试
    ...
    public AbstractClient(URL url, ChannelHandler handler) throws RemotingException {
        ...
        // The default reconnection interval is 2s, 1800 means warning interval is 1 hour. 默认重连接间隔为2s, 1800表示警告间隔为1小时。
...
//                删除线程池中已被取消的任务，不会被执行，当前线程如果被其他线程干扰不会成功
