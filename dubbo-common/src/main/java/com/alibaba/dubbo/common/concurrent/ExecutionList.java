 ...
 * logged.
 * 侦听器列表，每个侦听器都有一个关联的执行器，它保证在调用execute()之后执行添加的每个可运行程序。在执行调用之后添加的任何Runnable仍然保证执行。但是，不能保证侦听器将按照添加它们的顺序执行。
...
public final class ExecutionList {
     ...
     * 可运行的执行程序对要执行。它充当一个通过ExecutionList.RunnableExecutorPair执行的堆栈。下一个字段。
     */
     ...
     * in other executors.
     * 将可运行的和附带的执行程序添加到要执行的侦听器列表中。如果执行已经开始，侦听器将立即执行。

     注意:对于在任何线程中执行都是安全的快速、轻量级侦听器，请考虑more executor . samethreadexecutor。对于较重的侦听器，sameThreadExecutor()有一些警告:首先，侦听器运行的线程取决于在添加ExecutionList时是否执行了它。
     其次，调用execute的线程可能是一个内部实现线程，例如RPC网络线程，而sameThreadExecutor()侦听器可能在这个线程中运行。最后，在执行sameThreadExecutor侦听器期间，将阻止所有其他已注册但未执行的侦听器运行，即使这些侦听器将在其他执行器中运行。
     */
     ...
     * 运行此执行列表，按添加的顺序执行所有现有对。但是，请注意，在这一点之后添加的侦听器可能在之前添加的侦听器之前执行，并且请注意，所有侦听器的执行顺序最终由提供的执行器的实现选择。
    这个方法是幂等的。并行调用它几次在语义上等价于只调用它一次。
     */
    public void execute() {
     ...
     * 将给定的runnable提交给给定的执行程序，捕获并记录执行程序抛出的所有运行时异常。
     */
