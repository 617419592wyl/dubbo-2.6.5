...
//                如果地址是多个，拼接成 ip?backup=ip,ip
                url += "?" + Constants.BACKUP_KEY + "=" + backup.toString();
            }
        }
        String defaultProtocol = defaults == null ? null : defaults.get("protocol");
//        如果protocol属性值为空，默认dubbo协议
...
//        默认是proviger这个类
    ...
    /**
     * Check if the given value matches the given pattern. The pattern supports wildcard "*".检查给定值是否与给定模式匹配。该模式支持通配符“*”。
     *
