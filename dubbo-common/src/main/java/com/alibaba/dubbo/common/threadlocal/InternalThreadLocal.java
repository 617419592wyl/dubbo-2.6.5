 ...
 *
 * ThreadLocal的一个特殊变体，当从一个内部线程访问时，它会产生更高的访问性能。
 在内部，内部线程使用数组中的常量索引，而不是使用哈希码和哈希表来查找变量。虽然看起来非常微妙，但它与使用哈希表相比还是有一些性能优势，并且在频繁访问时非常有用。
 本设计是在Netty中学习的。
 */
public class InternalThreadLocal<V> {
    ...
    /**
     * Returns the number of thread local variables bound to the current thread.返回绑定到当前线程的线程本地变量的数量。
     */
     ...
     * 将值设置为未初始化;对get()的后续调用将触发对initialValue()的调用。
     */
    @SuppressWarnings("unchecked")
     ...
     * 将指定线程本地映射的值设置为未初始化;对get()的后续调用将触发对initialValue()的调用。指定的线程本地映射必须是针对当前线程的。
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns the initial value for this thread-local variable.返回该线程本地变量的初始值。
     */
    ...
    /**
     * Invoked when this thread local variable is removed by {@link #remove()}.通过remove()删除线程本地变量时调用。
     */
