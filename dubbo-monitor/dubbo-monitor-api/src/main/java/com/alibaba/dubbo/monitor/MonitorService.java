...
public interface MonitorService {
     ...
     * 收集监控数据1.1主机、应用程序、接口、组、版本、方法:记录源主机/应用程序/接口/方法1.2从用户发送的数据，添加源用户地址参数;否则，在参数1.3成功、失败、消耗中添加源用户地址:记录成功计数、失败计数和成功调用的总成本、平均成本(总成本/成功调用)
     *
     * @param statistics
     */
     ...
     * 查找监视器数据支持按日查找:count://主机/接口?1.1主机、应用程序、接口、组、版本、方法:按主机、应用程序、接口、方法查找的查询标准。当一个条件不存在时，表示所有条件都会被接受，但对于主机1.2 side=consumer,provider来说，0.0.0.0是所有条件:决定从哪一方返回的数据，默认情况下，
     * 提供者和使用者都是从哪一方返回的数据。view=chart将返回显示全天趋势的URL地址，方便嵌入其他网页1.4 date=2012-07-03:指定收集数据的日期，今天为默认值
     *
     * @param query
     * @return statistics
     */
